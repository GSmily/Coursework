...
start:-
	here(Here),
	print_list(['(Type help for a list of commands)']),
	print_list(['You start your quest in your ',Here]).
...
content_locations(X,Y):-location(X,Y).
content_locations(X,Y):-location(Y,X).

show_connection_names(X):-is_connected(X,Y),
	name(Y,N),write(N),write('\n'),fail.
show_connection_names(_).

show_connection_descriptions(X):-is_connected(X,Y),
	short_desc(Y,S),write(S),write('\n'),fail.
show_connection_descriptions(_).

show_content_names(X):-content_locations(X,Y),
	name(Y,N),write(N),write('\n'),fail.
show_content_names(_).

show_content_descriptions(X):-content_locations(X,Y),
	short_desc(Y,S),write(S),write('\n'),fail.
show_content_descriptions(_).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Look Commands:

u_look(Location):-
	write('Name:\n'),name(Location,N),write(N),
	write('\n\nLong Description:\n'),long_desc(Location,L),write(L),
	write('\n\nConnection Names:\n'),show_connection_names(Location),
	write('Connection Descriptions:\n'),show_connection_descriptions(Location),
	write('Name of Contents:\n'),show_content_names(Location),
	write('Description of Contents:\n'),show_content_descriptions(Location).
...
check_look(Location):-
	here(Here),
	is_equal(Location, Here),!.
check_look(Location):-
	print_list(['You can not see the ',Location,' from here']),fail.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Study Commands:

u_study(Object):-
	write('Object Description:\n'),long_desc(Object,L),write(L),
	write('\n\nName of Contents:\n'),show_content_names(Object),
	write('Description of Contents:\n'),show_content_descriptions(Object).

study(Object):-
	checked_study(Object),
	write('Object Description:\n'),long_desc(Object,L),write(L),
	write('\n\nName of Contents:\n'),show_content_names(Object),
	write('Description of Contents:\n'),show_content_descriptions(Object).
...
checked_study(Object):-
	here(Here),
	is_equal(Object, Here),!.
checked_study(Object):-
	print_list(['You can not study the ',Object,' from here']),fail.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Inventory Commands:

inventory:-
	write('Your inventory contains: '),nl,
	inventory_list.
inventory:-
...
check_move(Room):-
	here(Here),
	is_connected(Here, Room),!.
check_move(Room):-
	print_list(['You can not get to the ',Room,' from here']),fail.
...
u_take(Item):-
	checked_heavy(Item),
	move(Item,has),
	print_list(['You now have the ',Item]).
...
checked_take(Item):-
	...
	contains(Item,Here),!.          
checked_take(Item):-
	print_list(['There is not a ',Item,' here']),
	fail.
...
checked_heavy(Item):-                
	heavy(Item),
	print_list(['The ',Item,' is too heavy']),!,fail.
checked_heavy(_).                     
...
u_put(Item, Location):-
	has(Item),
	retract(has(Item)),
	asserta(location(Item,Location)).
u_put(Item):-
	print_list(['You don''t have a ',Item,' in your inventory']).

put(Item):-
	has(Item),
	here(Here),
	retract(has(Item)),
	asserta(location(Item,Here)).
put(Item):-
	print_list(['You don''t have a ',Item,' in your inventory']).
...
:- discontiguous u_make/2.

u_make(cooked_spaghetti):-
	has(spaghetti), has(sauce),
	retract(has(spaghetti)), retract(has(sauce)),
	asserta(has(cooked_spaghetti)),
	write('You cooked the spaghetti and sauce together to make delicious Cooked Spaghetti!'),nl,
	print_list([cooked_spaghetti,' was added to your inventory']).

u_make(charged_bone):-
	has(bone),
	retract(has(bone)),
	asserta(has(charged_bone)),
	write('You put the bone in front of the laser and created a Charged Bone!'),nl,
	print_list([charged_bone,' was added to your inventory']).

u_make(potion):-
	has(flask), has(charged_bone), has(fly),
	retract(has(flask)), retract(has(charged_bone)), retract(has(fly)),
	asserta(has(potion)),
	write('You brewed the Charged Bone and Fly together in a Flask to make a Potion!'),nl,
	print_list([potion,' was added to your inventory']).

u_make(clean_clothes):-
	has(dirty_clothes), has(laundry_soap),
	retract(has(dirty_clothes)), retract(has(laundry_soap)),
	asserta(has(clean_clothes)),
	write('You put the Dirty Clothes along with Laundry Soap in the Washing Machine and Cleaned the Clothes!'),nl,
	print_list([clean_clothes,' was added to your inventory']).
...
make(potion):-
	check_chemistry_lab(chemistry_lab),
	has(flask), has(charged_bone), has(fly),
	retract(has(flask)), retract(has(charged_bone)), retract(has(fly)),
	asserta(has(potion)),
	write('You brewed together the Charged Bone and Fly together in a Flask to make a Potion!'),nl,
	print_list([potion,' was added to your inventory']).

make(clean_clothes):-
	check_laundry_room(laundry_room),
	has(dirty_clothes), has(laundry_soap),
	retract(has(dirty_clothes)), retract(has(laundry_soap)),
	asserta(has(clean_clothes)),
	write('You put the Dirty Clothes along with Laundry Soap in the Washing Machine and Cleaned the Clothes!'),nl,
...
check_kitchen(Kitchen):-
	here(Here),
	is_equal(Kitchen, Here),!.
check_kitchen(Kitchen):-
	print_list(['You must be in the ',Kitchen,' to make ',cooked_spaghetti]),fail.

check_laser_lab(Laser):-
	here(Here),
	is_equal(Laser, Here),!.
check_laser_lab(Laser):-
	print_list(['You must be in the ',Laser,' to make ',charged_bone]),fail.

check_chemistry_lab(Chemistry):-
	here(Here),
	is_equal(Chemistry, Here),!.
check_chemistry_lab(Chemistry):-
	print_list(['You must be in the ',Chemistry,' to make ',potion]),fail.

check_laundry_room(Laundry):-
	here(Here),
	is_equal(Laundry, Here),!.
check_laundry_room(Laundry):-
	print_list(['You must be in the ',Laundry,' to make ',clean_clothes]),fail.
...
:- discontiguous transfer/2.

transfer(Disk, pylon_a):-
	move_disk_pylon_a(Disk,pylon_a).

transfer(Disk, pylon_b):-
	move_disk_pylon_b(Disk,pylon_b).

transfer(Disk, pylon_c):-
	move_disk_pylon_c(Disk,pylon_c).

move_disk_pylon_a(Disk,pylon_a):-
	retract(location(Disk,_)),
	asserta(location(Disk,pylon_a)).

move_disk_pylon_b(Disk,pylon_b):-
	retract(location(Disk,_)),
	asserta(location(Disk,pylon_b)).

move_disk_pylon_c(Disk,pylon_c):-
	retract(location(Disk,_)),
	asserta(location(Disk,pylon_c)).